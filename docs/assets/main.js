(function () {
  ...
  runBtn.addEventListener('click', function () {
    ...
    var script = [
      ...
      '#',
      '# IMPORTANT: place this file in your sqout checkout directory, then:',
      '#   chmod +x sqout-run.sh && ./sqout-run.sh',
      '#',
      '# It writes config/sqout.yaml and .env into the current directory',
      '# and runs `sqout run`. API keys are embedded — keep local, never commit.',
      '',
      'SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"',
      'cd "$SCRIPT_DIR"',
      '',
      '# Verify we are in a sqout checkout.',
      'if ! grep -qF \'name = "sqout"\' pyproject.toml 2>/dev/null; then',
      '  echo "❌ pyproject.toml not found in $SCRIPT_DIR."',
      '  echo "   Place this script in your sqout checkout directory and re-run."',
      '  exit 1',
      'fi',
      '',
      '# Activate venv if present, otherwise assume sqout is on PATH.',
      'if [ -f .venv/bin/activate ]; then',
      '  echo "🐍 Activating .venv ..."',
      '  source .venv/bin/activate',
      'elif sqout --help &>/dev/null; then',
      '  : # sqout already on PATH',
      'else',
      '  echo "❌ sqout not found in PATH and no .venv/bin/activate."',
      '  echo "   Create a venv first:  uv venv --python 3.11 && uv pip install -e ."',
      '  exit 1',
      'fi',
      '',
